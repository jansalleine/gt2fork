...
void printstatus(void)
{
  ...
  if (!menu)
  {
    ...
    if (usefinevib)
      printtext(40+10, 0, CSTATUSBAR, "FV");

    if (optimizepulse)
      printtext(43+10, 0, CSTATUSBAR, "PO");

    if (optimizerealtime)
      printtext(46+10, 0, CSTATUSBAR, "RO");

    if (ntsc)
      printtext(49+10, 0, CSTATUSBAR, "NTSC");
    else
      printtext(49+10, 0, CSTATUSBAR, " PAL");

    if (!sidmodel)
      printtext(54+10, 0, CSTATUSBAR, "6581");
    else
      printtext(54+10, 0, CSTATUSBAR, "8580");

    sprintf(textbuffer, "HR:%04X", adparam);
    printtext(59+10, 0, CSTATUSBAR, textbuffer);
    if (eamode) printbg(62+10+eacolumn, 0, cc, 1);

    if (multiplier)
    {
      sprintf(textbuffer, "%2dX", multiplier);
      printtext(67+10, 0, CSTATUSBAR, textbuffer);
    }
    else printtext(67+10, 0, CSTATUSBAR, "25Hz");
