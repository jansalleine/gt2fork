...
char *programname = "$VER: GoatTracker v2.76 Stereo";
  ...
  for (c = 1; c < argc; c++)
  {
    ...
    #endif
    {
      ...
      switch (argv[c][1]) //switch (toupper(argv[c][1]))
      {
        ...
        printtext(0,y++,15,"-Ixx Set reSID/-FP settings (0 = reSID Fast Resample, 1 = reSID Resample, 2 = reSID-FP Interpolate, 3 = reSID-FP Interpolate Resample) DEFAULT = reSID Resample");
